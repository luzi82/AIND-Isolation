...
def get_train(sample_var_dict,var_dict,arg_dict):
    loss_v_tf = get_loss_v_tf(sample_var_dict, var_dict, get_buffer_var_dict(var_dict), arg_dict)
...
def get_loss_v_tf(sample_tf_dict,model_lhs_dict,model_rhs_dict,arg_dict):
    mid0 = tf.one_hot(sample_tf_dict['choice_0'], OUTPUT_COUNT, axis=-1, dtype=tf.float32)
    mid0 = mid0 * get_q(sample_tf_dict['state_0'],model_lhs_dict)
    mid0 = tf.reduce_sum(mid0, reduction_indices=[1])

    mid1 = get_q(sample_tf_dict['state_1'],model_rhs_dict)
    mid1 = mid1 + tf.constant(-100., dtype=tf.float32) * ( tf.constant(1., dtype=tf.float32) - sample_tf_dict['choice_mask_1'] )
    mid1 = tf.reduce_max(mid1, reduction_indices=[1])  
    mid1 = mid1 * sample_tf_dict['cont_1']
    mid1 = mid1 * tf.constant(TRAIN_BETA)

    mid = mid0+mid1-sample_tf_dict['reward_1']
    mid = tf.abs(mid)
...
class DeepLearn(object):
    ...
    def __init__(self,arg_dict):
        ...
        self.cal_loss_v_tf = get_loss_v_tf(self.sample_input_ph_dict, self.var_dict, self.var_dict, self.arg_dict)
...
#             }, score[choice_0]
    ...
    def cal_loss_v(self, train_list_dict):
        ...
        return self.sess.run(self.cal_loss_v_tf,feed_dict=feed_dict)
    ...
    def get_move(self, game, legal_moves, time_left):
        ...
        if len(legal_moves)<=8:
            ...
            if len(legal_moves)>1:
                loss_np_v = self.dl.cal_loss_v(train_list_dict)
                weight_v = loss_np_v + 0.05
