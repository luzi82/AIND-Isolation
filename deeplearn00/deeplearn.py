...
def new_var_dict():
    stddev = 0.4
    ret = {}
    ret['w0']=tf.Variable(tf.random_normal([147,112] ,stddev=stddev,dtype=tf.float32))
    ret['b1']=tf.Variable(tf.random_normal([112]     ,stddev=stddev,dtype=tf.float32))
    ret['w2']=tf.Variable(tf.random_normal([112,85]  ,stddev=stddev,dtype=tf.float32))
    ret['b3']=tf.Variable(tf.random_normal([85]      ,stddev=stddev,dtype=tf.float32))
    ret['w4']=tf.Variable(tf.random_normal([85,65]   ,stddev=stddev,dtype=tf.float32))
    ret['b5']=tf.Variable(tf.random_normal([65]      ,stddev=stddev,dtype=tf.float32))
    ret['w6']=tf.Variable(tf.random_normal([65,49]   ,stddev=stddev,dtype=tf.float32))
    ret['b7']=tf.Variable(tf.random_normal([49]      ,stddev=stddev,dtype=tf.float32))
    return ret
...
def new_train_ph_dict():
    ...
    ret['choice_mask_1']=tf.placeholder(tf.float32,[None,49])
...
def get_train(train_ph_dict,var_dict,var_ph_dict,arg_dict):
    mid0 = tf.one_hot(train_ph_dict['choice_0'], 7*7, axis=-1, dtype=tf.float32)
    ...
    mid1 = mid1 + tf.constant(-100., dtype=tf.float32) * ( tf.constant(1., dtype=tf.float32) - train_ph_dict['choice_mask_1'] )
    mid1 = tf.reduce_max(mid1, reduction_indices=[1])  
    mid1 = mid1 * train_ph_dict['cont_1']
...
class DeepLearn(object):
    ...
    def __init__(self,arg_dict):
        ...
        self.random_t = tf.random_normal([7*7], stddev=arg_dict['random_stddev'])
        self.mask = tf.placeholder(tf.float32, [None,7*7])
...
class DLPlayer(object):
    ...
    def get_move(self, game, legal_moves, time_left):
        ...
        train_dict, _ = self.dl.cal_choice(state_0, self.get_choice_mask(legal_moves), self.train_enable)
        ret_move = self.idx_to_rc(train_dict['choice_0'])

        game_1 = game.forecast_move(ret_move)
        train_dict['state_1']       = self.get_state(game_1)
        train_dict['choice_mask_1'] = self.get_choice_mask(game_1.get_legal_moves())
    ...
    def get_choice_mask(self, legal_moves):
        ret_vv = np.zeros(7*7,dtype=np.float)
        for move in legal_moves:
            ret_vv[self.rc_to_idx(move)] = 1.
