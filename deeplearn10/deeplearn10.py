...
def get_train(sample_var_dict,var_dict,arg_dict):
    ...
    loss = score_diff
#     train = tf.train.GradientDescentOptimizer(0.03).minimize(loss,var_list=list(var_dict.values()))
    train = tf.train.AdamOptimizer().minimize(loss,var_list=list(var_dict.values()))
#     train = tf.train.AdadeltaOptimizer(0.03).minimize(loss,var_list=list(var_dict.values()))
...
class DeepLearn(object):
    ...
    def __init__(self,arg_dict):
        ...
        self.var_key_list = sorted(self.var_dict.keys())
        self.queue = {
    ...
    def init_delta_stat(self):
        self.old_model_var_dict = {}
        self.delta_sum_model_var_list = []
        
        self.assign_old_model_op_list = []
        self.add_delta_sum_op_list = []
        self.delta_mean_model_var_list = []
        self.reset_delta_sum_op_list = []
        
        for k in self.var_key_list:
            var = self.var_dict[k]
            old_model_var = tf.Variable(tf.zeros(var.get_shape(), dtype=tf.float32),trainable=False)
            delta_sum_model_var = tf.Variable(0.,trainable=False)
            
            self.old_model_var_dict[k] = old_model_var
            self.delta_sum_model_var_list.append(delta_sum_model_var)

            self.assign_old_model_op_list.append(tf.assign(old_model_var,var))
            self.add_delta_sum_op_list.append(tf.assign_add(delta_sum_model_var, tf.reduce_mean(tf.abs(var-old_model_var))))
            self.delta_mean_model_var_list.append(delta_sum_model_var)
            self.reset_delta_sum_op_list.append(tf.assign(delta_sum_model_var, 0.))
...
#             }, score[choice_0]
    ...
    def do_train(self):
        ...
        if self.push_done < self.arg_dict['train_memory']:
            return None
        self.sess.run(self.assign_old_model_op_list)
        _, loss, score_diff = self.sess.run([self.train,self.loss,self.score_diff])
        self.sess.run(self.add_delta_sum_op_list)
        if logging.getLogger().isEnabledFor(logging.DEBUG):
        ...
        if self.train_count % 1000 == 0:
            ...
            delta_mean_list = self.sess.run(self.delta_mean_model_var_list)
            delta_mean_list = [i.item() for i in delta_mean_list]
            delta_mean_dict = dict(zip(self.var_key_list, delta_mean_list))
            ...
            self.sess.run(self.reset_delta_sum_op_list)
        if logging.getLogger().isEnabledFor(logging.DEBUG):
...
#                 self.dl.push_train_dict(train_dict)
            ...
            for i in range(len(legal_moves)):
                ...
                good = good or ((loss_np_v[i]>=self.train_loss/PHI) and (train_dict_list[i]['cont_1']==0))
                if not good:
                    continue
                if logging.getLogger().isEnabledFor(logging.DEBUG):
