...
def get_train(train_ph_dict,var_dict,var_ph_dict,arg_dict):
    mid0 = tf.one_hot(train_ph_dict['choice_0'], OUTPUT_COUNT, axis=-1, dtype=tf.float32)
...
class DLPlayer(object):
    ...
    def get_move(self, game, legal_moves, time_left):
        ...
        if len(legal_moves)<=8:
            ...
            train_dict, _ = self.dl.cal_choice(state_0, self.get_choice_mask(game), self.train_enable)
            ret_move = self.idx_to_rc(game, train_dict['choice_0'])
